namespace ServerManagement.Models
{
    public static class ServersRepository
    {
        ...
        {
            return servers.Where(c => c.City.Equals(city, StringComparison.OrdinalIgnoreCase)).ToList();
        }

        public static Server? GetServersById(int serverId)
        {
            var server = servers.FirstOrDefault(c => c.ServerId == serverId);
            if (server != null)
            {
                return new Server
                {
        ...
        public static void UpdateServers(int serverId, Server server)
        {
            if (serverId != server.ServerId) return;

            var serverToUpdate = servers.FirstOrDefault(c => c.ServerId == serverId);
        ...
        public static void DeleteServers(int serverId)
        {
            var serverToRemove = servers.FirstOrDefault(c => c.ServerId == serverId);
        ...
        {
            return servers.Where(c => c.City.Contains(serverFilter, StringComparison.OrdinalIgnoreCase)).ToList();
        }
    }
}